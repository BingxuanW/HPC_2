...
/* About process */
static vector<tuple<int, int, double>> p_subtree_total;     // Combination of results of all processes
static vector<tuple<int, int, double>> p_subtree_final;     // Further integration
...
void Boruvkas(vector<pair<int, int>> &trees, vector<tuple<int, int, double>> Graph, int V, int E, vector<tuple<int, int, double>> &p_subtree);
...
int main(int argc, char **argv)
{
  ...
  if (pid == 0)
  {
    ...
    MPI_Gatherv(&p_subtree.front(), p_subtree_size, nodeType2, &p_subtree_total.front(), counts_recv, displs, nodeType2, 0, MPI_COMM_WORLD);
  }
  else
  {
    MPI_Gatherv(&p_subtree.front(), p_subtree_size, nodeType2, NULL, NULL, NULL, nodeType2, 0, MPI_COMM_WORLD);
  }
  ...
  if (pid == 0)
  {
    ...
    Boruvkas(trees2, p_subtree_total, VerticeNum, p_subtree_total.size(), p_subtree_final);
    for (int i = 0; i < p_subtree.size(); i++)
    {
      int rootIndex = find(trees2, get<0>(p_subtree_final[i]));
      tree_final.push_back(make_tuple(rootIndex, get<0>(p_subtree_final[i]), get<1>(p_subtree_final[i]), get<2>(p_subtree_final[i])));
    }
